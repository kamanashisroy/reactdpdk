...
template <>
tcp_fsmCallback nginz::tcp::make_tcpFsmCallback<TCP_STATE_CLOSING>()
{
    return [] (TcpControlBlock&tcb, TcpSegmentHeader&header, rte_autobuf m) {
        assert(tcb.state == TCP_STATE_CLOSING);
    };
}

template <>
tcp_fsmTmr nginz::tcp::make_tcpFsmTmr<TCP_STATE_CLOSING>()
{
